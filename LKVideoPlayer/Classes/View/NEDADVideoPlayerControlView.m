...
@interface NEDADVideoPlayerControlView ()
/** 加载loading */
...
@implementation NEDADVideoPlayerControlView

+ (instancetype)playerControlViewWithStatusModel:(NEDADVideoStatusModel *)playerStatusModel {
    NEDADVideoPlayerControlView *instance = [[self alloc] init];
...
- (void)showFastView:(NSInteger)draggedTime totalTime:(NSInteger)totalTime isForward:(BOOL)forawrd {
    ...
    if (forawrd) {
        self.fastImageView.image = [NEDADVideoPlayerImage imageNamed:@"ZFPlayer_fast_forward"];
    } else {
        self.fastImageView.image = [NEDADVideoPlayerImage imageNamed:@"ZFPlayer_fast_forward"];
    }
...
- (void)playDidEnd {
    ...
    self.backgroundColor  = UIColorFromRGB(0x8F8F8F);
    self.playeEnd         = YES;
    self.showing          = NO;
    // 隐藏controlView
...
- (void)playEndHideControlView {
    self.controlView.hidden = NO;
    [self.controlView playEndHideView:self.playeEnd];
}
...
- (NEDADVideoControlView *)controlView {
    if (!_controlView) {
        _controlView = [[NEDADVideoControlView alloc] init];
    }
    return _controlView;
}
...
- (UIButton *)repeatBtn {
    if (!_repeatBtn) {
        ...
        [_repeatBtn setImage:[NEDADVideoPlayerImage imageNamed:@"ZFPlayer_repeat_video"] forState:UIControlStateNormal];
    }
    return _repeatBtn;
}
...
- (UIView *)watchrRecordView {
    if (!_watchrRecordView) {
        ...
        _watchrRecordView.backgroundColor     = UIAlphaColorFromRGB(0x000000, 0.8);
...
- (UIButton *)closeWatchrRecordBtn {
    if (!_closeWatchrRecordBtn) {
        ...
        [_closeWatchrRecordBtn setImage:[NEDADVideoPlayerImage imageNamed:@"close"] forState:UIControlStateNormal];
    }
...
- (UIButton *)jumpPlayBtn {
    if (!_jumpPlayBtn) {
        ...
        [_jumpPlayBtn setTitleColor:UIColorFromRGB(0x3143AE) forState:UIControlStateNormal];
        _jumpPlayBtn.titleLabel.font = [UIFont systemFontOfSize:16.0];
    }
    return _jumpPlayBtn;
}

- (NEDADVideoProgressView *)progressView {
    if (!_progressView) {
        _progressView = [[NEDADVideoProgressView alloc] init];
