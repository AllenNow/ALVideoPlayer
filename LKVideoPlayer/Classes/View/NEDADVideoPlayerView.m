...
@interface NEDADVideoPlayerView ()<UIGestureRecognizerDelegate>
@property (nonatomic, weak) id<NEDVideoPlayerViewDelagate> delegate;
@property (nonatomic, weak) UIView *videoView;

// AVPlayerLayer视图容器层
@property (nonatomic, weak) UIView *playerLayerView;
// 控制，互动层
@property (nonatomic, strong) NEDADVideoPlayerControlView *playerControlView;
/** 播放器的参数模型 */
...
// !!!: 创建对象
+ (instancetype)videoPlayerViewWithSuperView:(UIView *)superview delegate:(id<NEDVideoPlayerViewDelagate>)delegate playerStatusModel:(NEDADVideoStatusModel *)playerStatusModel {
    
    NEDADVideoPlayerView *instance = [[NEDADVideoPlayerView alloc] init];
...
#pragma mark - getter
- (NEDADVideoPlayerControlView *)playerControlView {
    if (!_playerControlView) {
        _playerControlView = [NEDADVideoPlayerControlView playerControlViewWithStatusModel:self.playerStatusModel];
    }
    return _playerControlView;
}

- (NEDADVideoImageCoverView *)coverControlView {
    if (!_coverControlView) {
        _coverControlView = [[NEDADVideoImageCoverView alloc] init];
    }
    return _coverControlView;
}

- (NEDADVideoLoadingView *)loadingView {
    if (!_loadingView) {
