...
static const CGFloat LKPlayerAnimationTimeInterval             = 7.0f;
static const CGFloat LKPlayerControlBarAutoFadeOutTimeInterval = 0.35f;

@interface LKPlayerControlView ()
/** 加载loading */
...
/** 播放器的参数模型 */
@property (nonatomic, strong) LKPlayerStatusModel *playerStatusModel;
@end

@implementation LKPlayerControlView

+ (instancetype)playerControlViewWithStatusModel:(LKPlayerStatusModel *)playerStatusModel {
    LKPlayerControlView *instance = [[self alloc] init];
...
- (void)autoFadeOutControlView {
    ...
    [self performSelector:@selector(hideControl) withObject:nil afterDelay:LKPlayerAnimationTimeInterval];
}
...
- (void)showControl {
    ...
    [UIView animateWithDuration:LKPlayerControlBarAutoFadeOutTimeInterval animations:^{
...
- (void)showFastView:(NSInteger)draggedTime totalTime:(NSInteger)totalTime isForward:(BOOL)forawrd {
    ...
    if (forawrd) {
        self.fastImageView.image = [LKVideoPlayerImage imageNamed:@"ZFPlayer_fast_forward"];
    } else {
        self.fastImageView.image = [LKVideoPlayerImage imageNamed:@"ZFPlayer_fast_forward"];
    }
...
- (LKPortraitControlView *)portraitControlView {
    if (!_portraitControlView) {
        _portraitControlView = [[LKPortraitControlView alloc] init];
    }
    return _portraitControlView;
}

- (LKLandScapeControlView *)landScapeControlView {
    if (!_landScapeControlView) {
        _landScapeControlView = [[LKLandScapeControlView alloc] init];
    }
...
- (UIButton *)repeatBtn {
    if (!_repeatBtn) {
        ...
        [_repeatBtn setImage:[LKVideoPlayerImage imageNamed:@"ZFPlayer_repeat_video"] forState:UIControlStateNormal];
    }
    return _repeatBtn;
}
...
- (UIButton *)closeWatchrRecordBtn {
    if (!_closeWatchrRecordBtn) {
        ...
        [_closeWatchrRecordBtn setImage:[LKVideoPlayerImage imageNamed:@"close"] forState:UIControlStateNormal];
    }
...
- (UIButton *)jumpPlayBtn {
    if (!_jumpPlayBtn) {
        ...
        [_jumpPlayBtn setTitleColor:[UIColor colorWithHexString:@"#3143AE"] forState:UIControlStateNormal];
