...
@interface LKVideoPlayerView ()<UIGestureRecognizerDelegate>
@property (nonatomic, weak) id<LKVideoPlayerViewDelagate> delegate;
@property (nonatomic, weak) UIView *videoView;

// AVPlayerLayer视图容器层
@property (nonatomic, weak) UIView *playerLayerView;
// 控制，互动层
@property (nonatomic, strong) LKPlayerControlView *playerControlView;
/** 播放器的参数模型 */
...
@implementation LKVideoPlayerView
...
// !!!: 创建对象
+ (instancetype)videoPlayerViewWithSuperView:(UIView *)superview delegate:(id<LKVideoPlayerViewDelagate>)delegate playerStatusModel:(LKPlayerStatusModel *)playerStatusModel {
    
    LKVideoPlayerView *instance = [[LKVideoPlayerView alloc] init];
...
- (void)onDeviceOrientationChange {
    ...
    if (!LKBrightnessViewShared.isStartPlay || !_playerControlView) {
        return;
    }
...
#pragma mark - getter
- (LKPlayerControlView *)playerControlView {
    if (!_playerControlView) {
        _playerControlView = [LKPlayerControlView playerControlViewWithStatusModel:self.playerStatusModel];
    }
    return _playerControlView;
}

- (LKCoverControlView *)coverControlView {
    if (!_coverControlView) {
        _coverControlView = [[LKCoverControlView alloc] init];
    }
